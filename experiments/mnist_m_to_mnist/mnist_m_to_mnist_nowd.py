...
def train(batch_size,lr,K,lamb_clf,lamb_wd,lamb_centroid,lamb_sntg,epochs,LAMBDA,momentum):
    ...
    for epoch in range(1, epochs+1):
        ...
        if mean_accuracy > best_accu_t:
            ...
            torch.save(model_feature_source.state_dict(),'trained_models/wdgrl_source_clf1wd0.5c1s1_'+str(momentum)+'_nowd.pt')
            torch.save(model_feature_target.state_dict(),'trained_models/wdgrl_target_clf1wd0.5c1s1_'+str(momentum)+'_nowd.pt')
            torch.save(model_clf.state_dict(),'trained_models/wdgrl_clf_clf1wd0.5c1s1_'+str(momentum)+'_nowd.pt')
