...
def train(batch_size,lr,K,lamb_clf,lamb_wd,lamb_centroid,lamb_sntg,epochs,LAMBDA,momentum):
    ...
    ##########target data########################
    target_dataset_label = load_mnist_LS(source_num=1800,train_flag=True)
    ...
    for epoch in range(1, epochs+1):
        ...
        eval_dataset = load_mnist_LS(source_num=290,train_flag=False) #target_dataset_label
        ...
        if mean_accuracy > best_accu_t:
            ...
            torch.save(model_feature_source.state_dict(),'trained_models/wdgrl_source_clf1wd0.5c1s1_'+str(momentum)+'_nocentroid.pt')
            torch.save(model_feature_target.state_dict(),'trained_models/wdgrl_target_clf1wd0.5c1s1_'+str(momentum)+'_nocentroid.pt')
            torch.save(model_clf.state_dict(),'trained_models/wdgrl_clf_clf1wd0.5c1s1_'+str(momentum)+'_nocentroid.pt')
