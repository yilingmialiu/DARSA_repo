...
def main(args):
    ...
    clf_model = DenseNet(dim_in=616).to(device)
    clf_model.load_state_dict(torch.load('trained_models/DANN_source_seed_'+str(args.seed)+'.pt'))
    ...
    clf_optim = torch.optim.Adam(clf_model.parameters(), lr=1e-5)
