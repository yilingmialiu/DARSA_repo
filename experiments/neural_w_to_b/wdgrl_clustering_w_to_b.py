...
def train(batch_size,lr,K,lamb_clf,lamb_wd,lamb_centroid,lamb_sntg,epochs,LAMBDA,momentum):
    ...
    ##initialize models
    model_feature_source = DenseNet(dim_in=616).to(device)
    model_feature_target = DenseNet(dim_in=616).to(device)
    model_clf = Classifier(num_classes=3).to(device)
    path_to_model = 'trained_models/'
    model_feature_source.load_state_dict(torch.load(path_to_model+'source_feature_w_to_b.pt'))
    model_feature_target.load_state_dict(torch.load(path_to_model+'source_feature_w_to_b.pt'))
    model_clf.load_state_dict(torch.load(path_to_model+'source_clf_w_to_b.pt'))
    ...
    for epoch in range(1, epochs+1):
        ...
        with torch.no_grad():
                   save_parameters(mean_clf_loss_all=mean_clf_loss_all,mean_unweighted_clf_loss_all=mean_unweighted_clf_loss_all,\
                            ...
                            mean_w1_original_all = mean_w1_original_all,\
                            key_words='bo'+str(momentum))
