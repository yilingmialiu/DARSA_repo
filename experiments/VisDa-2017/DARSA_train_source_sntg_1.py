...
def do_epoch(model_feature,model_classifier, dataloader, criterion, optim=None):
    ...
    for x, y_true in tqdm(dataloader, leave=False):
        ...
        loss = loss + source_sntg_loss #0.01
...
def main(args):
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if target_accuracy > best_accuracy:
            ...
            }, f'trained_models/visda_source_model_best_sntg_1.pth')
            
        scheduler.step()

    # Load best model and evaluate
    print("\nEvaluating best model on target domain...")
    checkpoint = torch.load('trained_models/visda_source_model_best_sntg_1.pth')
