...
def create_dataloaders(batch_size):
    ...
    train_idx = shuffled_indices[:int(0.9*len(dataset))]
    val_idx = shuffled_indices[int(0.9*len(dataset)):]
...
def main(args):
    ...
    model_classifier = Classifier(input_dim=256).to(device)
    ...
    scheduler = StepLR(optim, step_size=15, gamma=0.1, verbose=True) #15
    ...
    for epoch in range(1, args.epochs+1):
        ...
        with torch.no_grad():
            ...
            if target_accuracy > best_accuracy:
                ...
                #torch.save(model_feature.state_dict(), 'trained_models/source_feature_rd_128_SGD_sntg_0_512.pt')
                #torch.save(model_classifier.state_dict(), 'trained_models/source_clf_rd_128_SGD_sntg_0_512.pt')
        
        #lr_schedule.step(target_accuracy)   
        scheduler.step()
