...
def train(batch_size, lr, K, lamb_clf, lamb_wd, lamb_centroid, lamb_sntg, epochs, LAMBDA, momentum, alpha, source_num, target_num, eval_target_num):
    ...
    model_feature_source.load_state_dict(torch.load(path_to_model+'source_feature_rd_128_SGD_alpha_'+str(alpha)+'.pt'))
    model_feature_target.load_state_dict(torch.load(path_to_model+'source_feature_rd_128_SGD_alpha_'+str(alpha)+'.pt'))
    model_clf.load_state_dict(torch.load(path_to_model+'source_clf_rd_128_SGD_alpha_'+str(alpha)+'.pt'))
    ...
    scheduler_source = ReduceLROnPlateau(feature_source_optim, mode='min', factor=0.1, patience=10, verbose=True)
    scheduler_target = ReduceLROnPlateau(feature_target_optim, mode='min', factor=0.1, patience=10, verbose=True)
    scheduler_clf = ReduceLROnPlateau(clf_optim, mode='min', factor=0.1, patience=10, verbose=True)

    # Example using ExponentialLR
    # scheduler_source = ExponentialLR(feature_source_optim, gamma=0.9)
    # scheduler_target = ExponentialLR(feature_target_optim, gamma=0.9)
    # scheduler_clf = ExponentialLR(clf_optim, gamma=0.9)

    # Example using StepLR (your original)
    ...
    for epoch in range(1, epochs+1):
        ...
        # For ReduceLROnPlateau, the scheduler step needs the validation metric
        # If you're using a different scheduler, adjust accordingly.
        # You'll need a validation set to use ReduceLROnPlateau effectively.
        # If you don't have a separate validation set, you can step the scheduler without a metric
        # but its behavior will be based on epoch number.
        # scheduler_source.step(mean_clf_loss) # Example if you decide to use source clf loss
        scheduler_source.step(mean_clf_loss)
        scheduler_target.step(mean_clf_loss)
        scheduler_clf.step(mean_clf_loss)
...
if __name__ == '__main__':
    ...
    print("--- Starting Run with Specific Hyperparameters ---")
    print("lr", lr)
