...
def train(batch_size, lr, K, lamb_clf, lamb_wd, lamb_centroid, lamb_sntg, epochs, LAMBDA, momentum, alpha, source_num, target_num, eval_target_num, pseudo_label_threshold):
    ...
    ##########target data########################
    #target_dataset_label = load_mnistm_LS_transform(target_num=target_num, alpha=alpha, MNISTM=MNISTM_transform, train_flag=True,
    #                                  transform=Compose([
    #                                      ToTensor(),
    #                                      RandomRotation(10), # Example augmentation
    #                                      RandomAffine(degrees=0, translate=(0.1, 0.1)) # Example augmentation
    #                                  ]))
    ...
    for epoch in range(1, epochs+1):
        ...
        for i in range(len_dataloader):
            ...
            #print("Number of confident pseudo-labels:", confident_mask.sum())
            ...
            if len(target_preds) > 0:
                ...
                cluster_t = torch.zeros((0, K)).to(device) # Empty tensor if no confident predictions
            ...
            if len(target_preds) > 0:
                centroid_loss = centroid_loss_func(K, device, source_y, target_y, source_feature, target_feature_masked)
            ...
            with torch.no_grad():
                ...
                if len(cluster_t) > 0:
                    ...
                    w_tgt_batch = torch.zeros(K).to(device) # Zero if no confident predictions
...
if __name__ == '__main__':
    ...
    K = 10
    batch_size = 1024
    lr = 0.005  # Example: Set a specific learning rate
    lamb_clf = 1.0 #0.9 # Example: Set a specific lambda_clf
    lamb_wd = 0.2  # Example: Set a specific lambda_wd
    lamb_centroid = 0.9
    lamb_sntg = 0.9
    LAMBDA = 30
    momentum = 0.5
