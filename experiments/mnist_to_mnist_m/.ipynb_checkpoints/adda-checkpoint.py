...
def main(args):
    ...
    source_dataset = load_mnist_LS(source_num=args.source_num,alpha=args.alpha,train_flag=True)
    source_loader = DataLoader(source_dataset, batch_size=half_batch,
                               shuffle=True, num_workers=0, pin_memory=True)
    
    target_dataset = load_mnistm_LS(target_num=args.target_num,alpha=args.alpha,MNISTM=MNISTM,train_flag=True)
    
    target_loader = DataLoader(target_dataset, batch_size=half_batch,
                               shuffle=True, num_workers=0, pin_memory=True)

    discriminator_optim = torch.optim.Adam(discriminator.parameters(),lr=0.00001)
    target_optim = torch.optim.Adam(target_model.parameters(),lr=0.00001)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        dataset_test = load_mnistm_LS(target_num=args.eval_target_num,alpha=args.alpha,MNISTM=MNISTM,train_flag=False)
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument('--source_num', type=int, default=600)
    arg_parser.add_argument('--target_num', type=int, default=100)
    arg_parser.add_argument('--eval_target_num', type=int, default=100)
