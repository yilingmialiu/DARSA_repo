...
seed_new = 1234 #1234 #15 #1234 #666 #123
...
def train(batch_size,lr,K,lamb_clf,lamb_wd,lamb_centroid,lamb_sntg,epochs,LAMBDA,momentum,alpha,source_num,target_num,eval_target_num):
    ...
    model_feature_source.load_state_dict(torch.load(path_to_model+'source_feature_rd_128_SGD_dominate.pt'))
    model_feature_target.load_state_dict(torch.load(path_to_model+'source_feature_rd_128_SGD_dominate.pt'))
    model_clf.load_state_dict(torch.load(path_to_model+'source_clf_rd_128_SGD_dominate.pt'))

    half_batch = batch_size // 2
    ##########source data########################
    source_dataset = load_mnist_dominate(source_num=source_num,digit=0,train_flag=True)
    source_loader = DataLoader(source_dataset, batch_size=half_batch, drop_last=True,\
                               shuffle=True, num_workers=0, pin_memory=True)
    
    ##########target data########################
    target_dataset_label = load_mnistm_dominate(target_num=target_num,digit=4,MNISTM=MNISTM,train_flag=True)

    target_loader = DataLoader(target_dataset_label, batch_size=half_batch,drop_last=True,\
                               shuffle=True, num_workers=0, pin_memory=True)
    
    ##########evaluation data########################
    eval_dataset = load_mnistm_dominate(target_num=eval_target_num,digit=4,MNISTM=MNISTM,train_flag=False)
    ...
    #scheduler_source = StepLR(feature_source_optim, step_size=70, gamma=0.1, verbose=True)
    #scheduler_target = StepLR(feature_target_optim, step_size=70, gamma=0.1, verbose=True)
    #scheduler_clf = StepLR(clf_optim, step_size=70, gamma=0.1, verbose=True)
    ...
    for epoch in range(1, epochs+1):
        ...
        if mean_accuracy > best_accu_t:
            ...
            test_dataset = load_mnistm_LS(target_num=100,alpha=alpha,MNISTM=MNISTM,train_flag=False)
            test_dataloader_all = DataLoader(test_dataset, batch_size=len(test_dataset), shuffle=True,
                                             drop_last=True, num_workers=0, pin_memory=True)
    
            total_accuracy = 0
            with torch.no_grad():
                for x, y_true in tqdm(test_dataloader_all, leave=False):
                    x, y_true = x.to(device), y_true.to(device)
                    h_t = model_feature_target(x)
                    y_pred = model_clf(h_t)
                    cluster_t = F.one_hot(torch.argmax(y_pred,1),num_classes=K).float()
                    ...
                    cluster_df = pd.DataFrame(cluster_t)
                    total_accuracy += (y_pred.max(1)[1] == y_true).float().sum().item()
                mean_accuracy = total_accuracy / len(test_dataset)
...
if __name__ == '__main__':
    ...
    train(batch_size=batch_size,lr=lr,\
          ...
          epochs=3000,LAMBDA=LAMBDA,momentum=momentum,\
          alpha = alpha,\
