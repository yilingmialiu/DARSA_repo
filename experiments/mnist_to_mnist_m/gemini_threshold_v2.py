...
def train(batch_size, lr, K, lamb_clf, lamb_wd, lamb_centroid, lamb_sntg, epochs, LAMBDA, momentum, alpha, source_num, target_num, eval_target_num, pseudo_label_threshold):
    ...
    model_feature_source.load_state_dict(torch.load(path_to_model+'source_feature_rd_128_SGD_alpha_'+str(alpha)+'_v2.pt'))
    model_feature_target.load_state_dict(torch.load(path_to_model+'source_feature_rd_128_SGD_alpha_'+str(alpha)+'_v2.pt'))
    model_clf.load_state_dict(torch.load(path_to_model+'source_clf_rd_128_SGD_alpha_'+str(alpha)+'_v2.pt'))
