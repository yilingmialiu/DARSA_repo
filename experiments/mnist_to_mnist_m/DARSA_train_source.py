...
def create_dataloaders(batch_size,alpha):
    ...
    target_dataset_label = load_mnistm_LS(target_num=100,alpha=alpha,MNISTM=MNISTM,train_flag=False)
...
def main(args):
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if val_accuracy > best_accuracy:
            ...
            torch.save(model_feature.state_dict(), 'trained_models/source_feature_rd_128_SGD_alpha_'+str(args.alpha)+'_v3.pt')
            torch.save(model_classifier.state_dict(), 'trained_models/source_clf_rd_128_SGD_alpha_'+str(args.alpha)+'_v3.pt')
